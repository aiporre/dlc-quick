...
class LabelWhiskersFrame(BaseFrame):
    def __init__(self, parent, config, config3d, sourceCam):
        ...
        # Spliting the frame into top and bottom panels.
    ...
    def browseDir(self, event):
        ...
        try:
            ...
            self.dataFrame = LabelWhiskersFrame.create_dataframe(
                self,
                self.dataFrame,
                self.relativeimagenames,
        ...
        if self.newimages:
            ...
            self.df = LabelWhiskersFrame.create_dataframe(
                self,
                None,
                self.newimages,
                self.individual_names,
...
def show(config, config3d, sourceCam, imtypes=["*.png"]):
    app = wx.App()
    frame = LabelWhiskersFrame(None, config, config3d, sourceCam).Show()
    app.MainLoop()
